...
if uploaded_file is not None:
    ...
    cv2_img = cv2.imdecode(np.frombuffer(bytes_data, np.uint8), cv2.IMREAD_UNCHANGED)
    ...
    if os.path.exists(hdf5_file):
        ...
        result = woman_categories[np.argmax(pred)]

        if result in woman_categories:
            image = Image.open("../faces/대표/여자/{}_대표.png".format(result))
            st.image(cv2_img)
